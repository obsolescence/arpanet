...
static void setup_connection (const char *hostname, int tx_port, int rx_port)
{
  struct hostent *h;

  h = gethostbyname (hostname);
  if (h == NULL) {
    fprintf (stderr, "Fatal error: gethostbyname failed for '%s'\n", hostname);
    exit (1);
  }

  destination.sin_family = AF_INET;
  destination.sin_port = htons (tx_port);
  memcpy (&destination.sin_addr, h->h_addr, h->h_length);
...
void imp_init_with_ports (const char *hostname, int tx_port, int rx_port)
{
  setup_connection (hostname, tx_port, rx_port);
  make_socket ();
  rx_sequence = tx_sequence = 0;
  imp_flags = imp_ready = 0;
}
