...
// Display positioning - use bottom 24 lines (calculated dynamically)
static int terminal_height = 24;  // Default for old terminals
static int status_line_row = 24;
...
static uint8_t imp_status[256];
static uint64_t imp_last_seen[256];
static uint8_t imp_reason[256];  // Reason codes for DEAD messages (type 7)

#define MAX_IMPS 256
static uint8_t known_imps[MAX_IMPS];
static int num_known_imps = 0;

// Configuration structures
typedef struct {
  uint8_t number;
  char name[32];
  uint8_t connected_imps[4];
  int num_connections;
  float x;
  float y;
} imp_info_t;
...
typedef struct {
  uint16_t tx_port;  // Host TX port (IMP RX port) - first number
  uint16_t rx_port;  // Host RX port (IMP TX port) - second number
  char hostname[32];
} host_interface_t;

// Display tracking (after typedef)
static int probe_cycle = 0;
static int total_messages = 0;
static imp_info_t global_imps[MAX_IMPS];
static int global_num_imps = 0;
static unsigned long global_last_probe_time = 0;
...
static int parse_section1(FILE *file, imp_info_t *imps, int max_imps) {
    char line[256];
    int count = 0;
    int in_section = 0;
    ...
    while (fgets(line, sizeof(line), file) != NULL) {
        ...
        if (in_section && strstr(line, "SECTION 2:") != NULL) {
            break;
        }
        ...
        // Parse IMP line: "IMP <number> #<name>"
        if (strncmp(line, "IMP ", 4) == 0) {
            if (count >= max_imps) {
                fprintf(stderr, "NCC: Too many IMPs in config\n");
                break;
            }

            int imp_num;
            char imp_name[32];
            if (sscanf(line, "IMP %d #%31s", &imp_num, imp_name) == 2) {
                imps[count].number = imp_num;
                strncpy(imps[count].name, imp_name, sizeof(imps[count].name) - 1);
                imps[count].name[sizeof(imps[count].name) - 1] = '\0';
                imps[count].num_connections = 0;
                count++;
            }
        }
    }
...
// Parse SECTION 3: PORT ASSIGNMENTS to find host interface for specific IMP
static int parse_section3_for_imp(FILE *file, uint8_t imp_number, host_interface_t *host_if) {
    char line[256];
    int in_section = 0;
    int in_target_imp = 0;
    uint8_t current_imp = 0;

    rewind(file);

    while (fgets(line, sizeof(line), file) != NULL) {
        // Check for section marker
        if (strstr(line, "SECTION 3: PORT ASSIGNMENTS") != NULL) {
            in_section = 1;
            continue;
        }

        // Exit if we hit the next section
        if (in_section && strstr(line, "SECTION 4:") != NULL) {
            break;
        }
        ...
        // Check for IMP line: "IMP <number> #<name>"
        if (strncmp(line, "IMP ", 4) == 0) {
            int imp_num;
            if (sscanf(line, "IMP %d", &imp_num) == 1) {
                current_imp = imp_num;
                in_target_imp = (current_imp == imp_number);
            }
            continue;
        }

        // If we're in the target IMP section, look for hi1 line
        if (in_target_imp && strstr(line, "hi1 ") != NULL) {
            // Skip commented lines
            if (line[0] == '#') {
                continue;
            }

            // Parse: "  hi1 <tx_port> <rx_port> host0 <hostname>"
            int tx_port, rx_port;
            char hostname[32];
            if (sscanf(line, " hi1 %d %d host%*d %31s", &tx_port, &rx_port, hostname) == 3) {
                host_if->tx_port = tx_port;
                host_if->rx_port = rx_port;
                strncpy(host_if->hostname, hostname, sizeof(host_if->hostname) - 1);
                host_if->hostname[sizeof(host_if->hostname) - 1] = '\0';
                return 1;  // Success
            }
        }
    }
...
// Parse SECTION 4: NODE COORDINATES
static void parse_section4(FILE *file, imp_info_t *imps, int num_imps) {
    char line[256];
    int in_section = 0;
    ...
    while (fgets(line, sizeof(line), file) != NULL) {
        // Check for section marker
        if (strstr(line, "SECTION 4: NODE COORDINATES") != NULL) {
            in_section = 1;
            continue;
        }
        ...
        // Parse IMP coordinate line: "IMP <number> <x> <y> #<name>"
        if (strncmp(line, "IMP ", 4) == 0) {
            int imp_num;
            float x, y;
            if (sscanf(line, "IMP %d %f %f", &imp_num, &x, &y) == 3) {
                // Find the IMP in our array and store coordinates
                for (int i = 0; i < num_imps; i++) {
                    if (imps[i].number == imp_num) {
                        imps[i].x = x;
                        imps[i].y = y;
                        break;
                    }
...
static void parse_topology_config(const char *filename, imp_info_t *imps, int *num_imps) {
    FILE *file = fopen(filename, "r");
    if (file == NULL) {
        fprintf(stderr, "NCC: Error opening topology config file '%s': %s\n",
                filename, strerror(errno));
        exit(1);
    }

    *num_imps = parse_section1(file, imps, MAX_IMPS);
    parse_section4(file, imps, *num_imps);

    // Also populate the known_imps array for probing
    num_known_imps = 0;
    for (int i = 0; i < *num_imps && i < MAX_IMPS; i++) {
        known_imps[num_known_imps++] = imps[i].number;
    }

    fclose(file);
    fprintf(stderr, "NCC: Loaded %d IMPs from topology config.\n", *num_imps);
}

static uint8_t packet[200];
static uint8_t app[200];

// Get terminal height and set display positioning
static void init_terminal_positioning(void) {
    struct winsize w;
    if (ioctl(STDOUT_FILENO, TIOCGWINSZ, &w) == 0 && w.ws_row > 0) {
        terminal_height = w.ws_row;
    }
    // Status line at row 2 (avoids scroll risk), map starts at row 3
    status_line_row = 2;
    display_offset = terminal_height - 22;  // Start map at row 3 (below status)
    fprintf(stderr, "NCC: Terminal height: %d rows, status at row 2, map offset: %d\n",
            terminal_height, display_offset);
}

// Display helper functions
static imp_info_t* find_imp_by_number(uint8_t imp_num, imp_info_t *imps, int num_imps) {
    for (int i = 0; i < num_imps; i++) {
        if (imps[i].number == imp_num) {
            return &imps[i];
        }
...
static void display_network_initial(imp_info_t *imps, int num_imps) {
    for (int i = 0; i < num_imps; i++) {
        // Invert Y coordinate: y = 20 - y (to flip map upside down)
        // Add offset to position in bottom 24 lines
        int row = (int)(20.0 - imps[i].y + 0.5) + 1 + display_offset;
        // Scale X coordinate from 60 to 72 columns (multiply by 1.2)
        int col = (int)(imps[i].x * 1.2 + 0.5) + 1;

        // Print IMP name (first 6 characters)
        printf("\033[%d;%dH", row, col);
        printf("%.6s", imps[i].name);

        // Print IMP number and initial message status
        printf("\033[%d;%dH", row + 1, col);
        printf("%02d--", imps[i].number);
    }
    fflush(stdout);
}

static void update_imp_message(uint8_t imp_num, uint8_t msg_type, imp_info_t *imps, int num_imps) {
    imp_info_t *imp = find_imp_by_number(imp_num, imps, num_imps);
    if (imp == NULL) {
        return;  // IMP not found in topology
    }

    // Invert Y coordinate: y = 20 - y (to flip map upside down)
    // Add offset to position in bottom 24 lines
    int row = (int)(20.0 - imp->y + 0.5) + 1 + display_offset;
    // Scale X coordinate from 60 to 72 columns (multiply by 1.2)
    int col = (int)(imp->x * 1.2 + 0.5) + 1;

    // Update message line with IMP number and message type
    printf("\033[%d;%dH", row + 1, col);
    if (msg_type == IMP_DEAD) {
        // For DEAD messages, show reason code: "01-70" (type 7, reason 0)
        printf("%02d-%d%d ", imp_num, msg_type, imp_reason[imp_num]);
    } else {
        printf("%02d-%d ", imp_num, msg_type);
    }
    fflush(stdout);
}

static void update_status_line(imp_info_t *imps, int num_imps, unsigned long time_tick, unsigned long last_probe_time) {
    // Count active IMPs
    int active_imps = 0;
    for (int i = 0; i < 256; i++) {
        if (imp_status[i] != 0) {
    ...
    printf("\033[%d;1H", status_line_row);
    printf("\033[K");  // Clear line
    printf("Cycle: %d | Active: %d/%d | Msgs: %d | Time: %lus",
           probe_cycle, active_imps, num_imps, total_messages,
           time_tick - last_probe_time);
    fflush(stdout);
}
...
static void process_imp (uint8_t *packet, int length)
{
  ...
  // Most messages with a source have it in the second byte.
  // This includes REGULAR, RFNM, DEAD, INCOMPLETE.
  // Source is not relevant for NOP, DOWN, RESET.
  if (type == IMP_REGULAR || type == IMP_RFNM || type == IMP_DEAD ||
      type == IMP_INCOMPL) {
    source_imp = packet[1];
  }

  // Extract reason code for DEAD messages (type 7)
  if (type == IMP_DEAD) {
    imp_reason[source_imp] = packet[3] & 0x0F;
  }

  // Store the status and timestamp
  imp_status[source_imp] = type;
  imp_last_seen[source_imp] = time_tick;

  // Update display
  if (source_imp != 0) {
    total_messages++;
    update_imp_message(source_imp, type, global_imps, global_num_imps);
    update_status_line(global_imps, global_num_imps, time_tick, global_last_probe_time);
  }
...
static void app_imp_status(void)
{
    uint8_t imp = app[1];
    uint8_t reply[10];

    reply[0] = WIRE_IMP_STATUS + 1;
    reply[1] = imp_status[imp];
    
    // Copy the 64-bit timestamp into the reply buffer
    uint64_t timestamp = imp_last_seen[imp];
    memcpy(&reply[2], &timestamp, sizeof(timestamp));

    if (sendto(fd, reply, sizeof(reply), 0, (struct sockaddr *)&client, len) == -1)
        fprintf(stderr, "NCC: sendto %s error: %s.\n", client.sun_path, strerror(errno));
}
...
static void application (void)
{
  ...
  case WIRE_IMP_STATUS: app_imp_status(); break;
...
static void cleanup (void)
{
  ...
  // Tell the IMP we're going down by clearing host ready bit
  fprintf(stderr, "NCC: Clearing host ready bit\n");
  imp_host_ready (0);

  // Give time for the message to be sent
  usleep(100000);  // 100ms

  // Close IMP socket to release ports
  fprintf(stderr, "NCC: Closing IMP connection\n");
  imp_shutdown();

  // Unix socket cleanup (currently disabled)
  // unlink (server.sun_path);

  fprintf(stderr, "NCC: Shutdown complete\n");
}
...
void ncp_init (void)
{
  // Unix socket disabled for now - may be used later for WebSocket bridge
  /*
  char *path;
  ...
  path = getenv ("NCP");
  if (path == NULL) {
    fprintf (stderr, "NCP: ERROR: NCP environment variable not set.\n");
    fprintf (stderr, "NCP: Please set NCP to a Unix socket path, for example:\n");
    fprintf (stderr, "NCP:   export NCP=/tmp/ncp.socket\n");
    exit (1);
  }
...
int main (int argc, char **argv)
{
  ...
  imp_info_t imps[MAX_IMPS];
  int num_imps = 0;
  host_interface_t host_if;
  uint8_t target_imp;
  ...
  // Validate arguments
  if (argc < 2 || argc > 3) {
    fprintf(stderr, "Usage: %s <imp_number> [topology_config_file]\n", argv[0]);
    fprintf(stderr, "\n");
    fprintf(stderr, "Arguments:\n");
    fprintf(stderr, "  <imp_number>              IMP number to attach to (required)\n");
    fprintf(stderr, "  [topology_config_file]    Path to topology config (optional)\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "If config file is not specified, searches for 'arpanet-topology.conf' in:\n");
    fprintf(stderr, "  1. Current directory (./)\n");
    fprintf(stderr, "  2. Parent directory (../)\n");
    fprintf(stderr, "  3. Grandparent directory (../../)\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "Examples:\n");
    fprintf(stderr, "  %s 32\n", argv[0]);
    fprintf(stderr, "  %s 32 /path/to/arpanet-topology.conf\n", argv[0]);
    exit(1);
  }

  target_imp = atoi(argv[1]);
  ...
  if (argc == 3) {
    ...
    for (int i = 0; i < 3; i++) {
      if (access(search_paths[i], R_OK) == 0) {
        strncpy(config_path, search_paths[i], sizeof(config_path) - 1);
    ...
    if (config_file == NULL) {
      fprintf(stderr, "NCC: ERROR: No topology config file found.\n");
      fprintf(stderr, "NCC: Searched in:\n");
      fprintf(stderr, "  - ./arpanet-topology.conf\n");
      fprintf(stderr, "  - ../arpanet-topology.conf\n");
      fprintf(stderr, "  - ../../arpanet-topology.conf\n");
      fprintf(stderr, "\n");
      fprintf(stderr, "NCC: Please provide config file path as second argument.\n");
      exit(1);
    }
  }

  fprintf(stderr, "NCC: Starting Network Control Center\n");
  fprintf(stderr, "NCC: Using config file: %s\n", config_file);
  fprintf(stderr, "NCC: Target IMP: %d\n", target_imp);

  // Parse the topology configuration
  parse_topology_config(config_file, imps, &num_imps);

  // Verify the target IMP exists in the topology
  int imp_found = 0;
  for (int i = 0; i < num_imps; i++) {
    if (imps[i].number == target_imp) {
      imp_found = 1;
      fprintf(stderr, "NCC: Found IMP %d (%s) in topology\n",
              imps[i].number, imps[i].name);
      break;
    }
  }

  if (!imp_found) {
    fprintf(stderr, "NCC: ERROR: IMP %d not found in topology configuration\n", target_imp);
    fprintf(stderr, "NCC: Available IMPs: ");
    for (int i = 0; i < num_imps; i++) {
      fprintf(stderr, "%d%s", imps[i].number, (i < num_imps - 1) ? ", " : "\n");
    }
    exit(1);
  }

  // Find host interface (hi1) for the target IMP
  FILE *file = fopen(config_file, "r");
  if (file == NULL) {
    fprintf(stderr, "NCC: Error re-opening config file: %s\n", strerror(errno));
    exit(1);
  }

  if (!parse_section3_for_imp(file, target_imp, &host_if)) {
    fprintf(stderr, "NCC: ERROR: No hi1 (host interface 1) found for IMP %d in configuration\n", target_imp);
    fprintf(stderr, "NCC: This IMP does not have a host interface defined.\n");
    fprintf(stderr, "NCC: Please check SECTION 3: PORT ASSIGNMENTS in the config file.\n");
    fclose(file);
    exit(1);
  }
  ...
  fprintf(stderr, "NCC: Host interface found: %s\n", host_if.hostname);
  fprintf(stderr, "NCC: TX Port (host->IMP): %d\n", host_if.tx_port);
  fprintf(stderr, "NCC: RX Port (host<-IMP): %d\n", host_if.rx_port);

  // Copy to global for display functions
  memcpy(global_imps, imps, sizeof(imp_info_t) * num_imps);
  global_num_imps = num_imps;

  // Initialize terminal positioning
  init_terminal_positioning();

  // Initialize IMP connection with parsed ports
  imp_init_with_ports("localhost", host_if.tx_port, host_if.rx_port);
  ...
  // Initial display - clear screen with VT-100
  printf("\033[2J\033[H");  // Clear screen and home cursor
  display_network_initial(global_imps, global_num_imps);
  global_last_probe_time = last_probe_time;
  update_status_line(global_imps, global_num_imps, time_tick, global_last_probe_time);
  fflush(stdout);
  ...
  for (;;) {
    ...
    struct timeval tv;
    FD_ZERO (&rfds);
    // FD_SET (fd, &rfds);  // Unix socket disabled - for future WebSocket bridge
    imp_fd_set (&rfds);
    tv.tv_sec = 1;
    tv.tv_usec = 0;
    ...
    if (n == -1)
      ...
      /* Unix socket disabled - for future WebSocket bridge
      if (FD_ISSET (fd, &rfds)) {
        application ();
      }
      */
    ...
    // Probe known IMPs every 10 seconds
    if (time_tick - last_probe_time >= 10) {
        probe_cycle++;

        // Clear screen for subsequent cycles
        if (probe_cycle > 1) {
            printf("\033[2J\033[H");
        }
        ...
        for (int i = 0; i < num_known_imps; i++) {
            ncp_rst(known_imps[i]);
        }

        last_probe_time = time_tick;
        global_last_probe_time = last_probe_time;
        update_status_line(global_imps, global_num_imps, time_tick, global_last_probe_time);
        fflush(stdout);
    }
